...
void window_init(struct rjd_window* window, const struct rjd_window_environment* env)
{
	...
	// resources
	{
		...
		// cube mesh
		{
			...
			float tints[NUM_VERTS * 4] = {0};
			for (size_t i = 0; i < NUM_VERTS * 4; i += 4) {
				const size_t num_floats_per_face_pair = 2 * 2 * 3 * 4;
				if (i < 1 * num_floats_per_face_pair) {
					tints[i + 0] = 1.0f;
					tints[i + 1] = 0.0f;
					tints[i + 2] = 0.0f;
					tints[i + 3] = 1.0f;
				} else if (i < 2 * num_floats_per_face_pair) {
					tints[i + 0] = 0.0f;
					tints[i + 1] = 1.0f;
					tints[i + 2] = 0.0f;
					tints[i + 3] = 1.0f;
				} else if (i < 3 * num_floats_per_face_pair) {
					tints[i + 0] = 0.0f;
					tints[i + 1] = 0.0f;
					tints[i + 2] = 1.0f;
					tints[i + 3] = 1.0f;
